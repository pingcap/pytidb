...
| `examples/` | End-to-end samples (CRUD, RAG, hybrid search, custom embeddings, Text2SQL, routing real-time data, etc.) that you can run against a TiDB cluster. |
| `docs/` | MkDocs site that mirrors <https://pingcap.github.io/ai>. Contains `mkdocs.yml`, its own `pyproject.toml`, and `src/` with landing pages and shared styles. |
...
- `client.py`: exposes `TiDBClient`, a SQLAlchemy-powered engine wrapper that knows how to connect, ensure databases, swap schemas (`use_database`), and list or manage tables. It is the entry point new apps instantiate.
...
- `table.py`: wraps a `TableModel` plus `TiDBClient` to provide CRUD, automatic vector/full-text index creation, and auto-embedding plumbing. It inspects `__pydantic_fields__`, attaches indexes, and exposes fluent APIs such as `insert`, `bulk_insert`, and `search`.
...
- `filters.py`: converts dict-like filters or SQLAlchemy expressions into executable clauses shared by both search and table APIs.
- `embeddings/`: abstract base classes plus built-in providers (OpenAI, Jina, etc.), dimension metadata, and utilities for converting sources or queries. Embedding functions attach to `VectorField` definitions or runtime search requests.
- `rerankers/`: base class plus LiteLLM-powered implementations for optional reranking during search.
- `fusion.py` and `result.py`: helper utilities for combining search outputs (RRF or weighted fusion) and normalizing SQL query results to `QueryResult`, `SQLModelQueryResult`, or simple dict/list conversions.
...
- `examples/*`: organized by scenario (`basic`, `rag`, `fulltext_search`, `memory`, `vector-search-with-realtime-data`, etc.). Each folder contains runnable scripts or notebooks that demonstrate how the higher-level abstractions should be used. When you add new functionality, consider dropping a runnable sample here.
...
1. **Client and database management** - `TiDBClient.connect()` creates SQLAlchemy engines, optionally ensures the target database, and exposes helpers (`create_database`, `list_tables`, `session()` context managers) shared by the rest of the stack.
