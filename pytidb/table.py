...
class Table(Generic[T]):
    ...
    def update(self, values: dict, filters: Optional[Filters] = None) -> Optional[T]:
        ...
        with self._client.session() as db_session:
            filter_clauses = (
                build_filter_clauses(filters, self._sa_table)
            ...
            target_instance = db_session.exec(target_stmt).first()

            stmt = update(self._table_model)
            if filter_clauses:
                stmt = stmt.filter(*filter_clauses)
            stmt = stmt.values(values)
            result = db_session.execute(stmt)

            if result.rowcount == 0 or target_instance is None:
                return None

            db_session.refresh(target_instance)
