...
class BGEM3EmbeddingFunction(BaseEmbeddingFunction):
    ...
    """
    use_fp16: bool = Field(True, description="Whether to use FP16 precision for faster inference")
    device: Optional[str] = Field(None, description="Device to run the model on (auto-detected if None)")
    ...
    def _init_model(self):
        ...
        if self._model is None:
            ...
            # Update dimensions based on actual model output
            test_output = self._model.encode("test", return_dense=True)
            actual_dims = test_output["dense_vecs"].shape[1]
            if self.dimensions != actual_dims:
                print(f"Updating dimensions from {self.dimensions} to {actual_dims}")
...
def test_bge_m3_embedding():
    """Test the BGEM3EmbeddingFunction implementation."""
    print("=== Testing BGE-M3 Embedding Function ===")
    
    # Initialize the embedding function
    embed_fn = BGEM3EmbeddingFunction(use_fp16=True)
    ...
    embedding = embed_fn.get_query_embedding(query, "text")
    ...
    batch_embeddings = embed_fn.get_source_embeddings(documents, "text")
    print(f"\nBatch processing {len(documents)} documents:")
    for i, doc in enumerate(documents):
        print(f"  Doc {i+1}: {len(batch_embeddings[i])} dimensions")
