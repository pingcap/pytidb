...
def render_mobile_ui(recommendations: List[Dict[str, Any]], user_profile: str):
    """Render the mobile shopping app UI"""
...
def render_admin_panel(table: Table):
    """Render the admin panel for managing products."""
...
def user_view_page():
    ...
    st.title("🛍️ User View (Shopping App)")
    st.markdown("Experience personalized product recommendations based on your preferences")
    
    # Settings and Shopping App in two columns
    settings_col, shopping_col = st.columns([1, 1])
...
def product_management_page():
    ...
    st.markdown("Manage your product catalog with real-time updates")
...
def main():
    ...
    # Create pages
    pages = {
        "User View (Shopping App)": user_view_page,
        "Product Management": product_management_page,
    }
