...
def test_hybrid_search_weighted_fusion_equal_weights_to_list(hybrid_table: Table):
    expected_results = [
        {"id": 1, "_distance": 0.46570, "_match_score": 1.40964, "_score": 0.69481},
        {"id": 2, "_distance": 0.60275, "_match_score": 0.49918, "_score": 0.50271},
    ]
    actual_results = (
        hybrid_table.search("AI database", search_type="hybrid")
        .text_column("description")
...
def test_hybrid_search_weighted_fusion_vs_weighted_to_list(hybrid_table: Table):
    expected_results = [
        {"id": 1, "_distance": 0.46570, "_match_score": 1.40964, "_score": 0.59122},
        {"id": 2, "_distance": 0.60275, "_match_score": 0.49918, "_score": 0.51861},
    ]
    actual_results = (
        hybrid_table.search("AI database", search_type="hybrid")
        .text_column("description")
        .fusion("weighted", vs_weight=0.7, fts_weight=0.3)
        .limit(2)
        .to_list()
    )
...
def test_hybrid_search_weighted_fusion_fts_weighted_to_list(hybrid_table: Table):
    expected_results = [
        {"id": 1, "_distance": 0.46570, "_match_score": 1.40964, "_score": 0.79840},
        {"id": 2, "_distance": 0.60275, "_match_score": 0.49918, "_score": 0.48682},
    ]
    actual_results = (
        hybrid_table.search("AI database", search_type="hybrid")
        .text_column("description")
        .fusion("weighted", vs_weight=0.3, fts_weight=0.7)
        .limit(2)
        .to_list()
    )
...
def test_hybrid_search_weighted_fusion_to_rows(hybrid_table: Table):
    expected_results = [
        {"id": 1, "_distance": 0.46570, "_match_score": 1.40964, "_score": 0.69481},
        {"id": 2, "_distance": 0.60275, "_match_score": 0.49918, "_score": 0.50271},
    ]
    ...
    for actual, expected in zip(actual_results, expected_results):
        ...
        assert abs(actual._mapping["_distance"] - expected["_distance"]) < 1e-3
        assert abs(actual._mapping["_match_score"] - expected["_match_score"]) < 1
        assert abs(actual._mapping["_score"] - expected["_score"]) < 1e-3


def test_hybrid_search_weighted_fusion_to_pydantic(hybrid_table: Table):
    expected_results = [
        {"id": 1, "_distance": 0.46570, "_match_score": 1.40964, "_score": 0.69481},
        {"id": 2, "_distance": 0.60275, "_match_score": 0.49918, "_score": 0.50271},
    ]
    ...
    for actual, expected in zip(actual_results, expected_results):
        ...
        assert abs(actual.distance - expected["_distance"]) < 1e-3
        assert abs(actual.match_score - expected["_match_score"]) < 1
        assert abs(actual.score - expected["_score"]) < 1e-3


def test_hybrid_search_weighted_fusion_to_pandas(hybrid_table: Table):
    expected_results = [
        {"id": 1, "_distance": 0.46570, "_match_score": 1.40964, "_score": 0.69481},
        {"id": 2, "_distance": 0.60275, "_match_score": 0.49918, "_score": 0.50271},
    ]
